...
```{r}
temp_names <- temp_libraries %>% unite(county_name, c("county", "state_name"), sep = " County, ") %>% select(county_name) 
temp_names[25,]
```
```{r}
county_incomes %>% separate_wider_delim(NAME, delim = ", ", names = c("county", "state"))
```
