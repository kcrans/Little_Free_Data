...
Some of the rows are for foreign libraries, but it look like the majority are libraries with no street entries. Some like charter number G10014(148 Marina Plaza	Dunedin) are located very close to the ocean and hence were classified incorrectly due to the resolution of the geography. A few like 150219 (1710 S Trenon Ave Tulsa) are mislabeled with coordinates not in the Unites States. Luckily there are only 123 entries, so we can filter them out without major concern.

```{r}
libraries <- libraries %>% mutate(alt_state = state_names[alt_classifications])
```

If we take a look at the state assignments based off the geolocation coordinates ("state") and MapAnything coordinates ("alt_state"), the possibilities are:

- NA for both, which means the library is not in the U.S and should be removed from our dataframe.

- An actual state for the geolocation coordinates and NA for MapAnything. We should use the "state" assignment then.

- An actual state for the MapAnything coordinates and NA for the geolocation. We will go with the "alt_state" assignment in this case.

- Actual states for both coordinates. If they are the same, we will go with that assignment. If they are different, we will use whichever assignment lines up with the "State_Province_Region__c" value for that datapoint.

long, lat, actual_state

The library is not in the U.S., so it will have NA values for both coordinates.

```{r}
libraries %>% filter(state != alt_state) %>% select(State_Province_Region__c, state, alt_state)
```
