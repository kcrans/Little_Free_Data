...
```{r}
temp_libraries %>% mutate(State_Province_Region__c = convert_state(State_Province_Region__c)) 
...
```{r}
state_pops = as_tibble(read_csv("census_data.csv"))
...
st_transform(us_states, crs = 3857) %>% left_join(norm_state_counts, by = join_by(NAME)) %>%
  ...
  labs(fill = "Number of LFLs") + 
  scale_fill_fermenter(n.breaks = 9, direction = 1, palette = "Oranges") +
  ggtitle("Number of Little Free Libraries per 10,000 residents by state")
```
