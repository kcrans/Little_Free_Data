...
1.
```{r}
libraries_1 <- temp_libraries
libraries_1 <- libraries_1 %>% add_column(state_name = NA, long = NA, lat = NA)
...
case_1 <- temp_libraries %>% filter(is.na(state) & is.na(alt_state)) %>% filter(!is.na(State_Province_Region__c)) %>%
  filter(City__c != "Alytus") %>% mutate(state_name = State_Province_Region__c, long = Library_Geolocation__Longitude__s, lat = Library_Geolocation__Latitude__s)
```
...
4.
```{r}
case_4 <- temp_libraries %>% filter(state == alt_state)
```

5.
```{r}
temp_libraries %>% filter(state != alt_state)
```

Joining it all together:

```{r}
temp_lib <- bind_rows(case_1, case_2, case_3, case_4)
```
