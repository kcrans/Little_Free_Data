...
These (0, 0) coordinates are basically missing values as all the libraries we are looking at are located in the US and thus they are invalid coordinates If we look at the webapp, it appears that the developers use the geolocation values on the interactive map:
...
state_names <- states_t$NAME_1
classifications <- as.integer(st_intersects(pts_t, states_t))
...
```{r}
libraries %>% group_by(state) %>% count
```
Research Q's:
