...
```{r}
temp_names <- temp_libraries  %>% unite(full_name, c("county", "state_name"), sep = ", ")  %>% 
  ...
  select(City__c, Postal_Zip_Code__c, full_name) #%>%  %>%
 # unique()

```

```{r}
temp_comp <- county_incomes %>% separate_wider_delim(NAME, delim = ", ", names = c("county", "state")) %>% select(county, state)
```

```{r}
temp_names %>% filter(!(tolower(full_name) %in% tolower(county_incomes$NAME)))
```

```{r}
temp_libraries %>% filter(City__c == "Marinette")
```

```{r}
county_incomes %>% filter(NAME == "Allegheny County, Pennsylvania")
temp_names %>% filter(full_name == "Allegheny County, Pennsylvania")
```



```{r}
temp_comp %>% filter(state == "Virginia")
temp_libraries %>% filter(county == "Clifton Forge City")
```
